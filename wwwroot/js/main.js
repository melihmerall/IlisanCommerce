  ...
  (function($) {
    ...
    $('.input-quantity-product-detail').each(function() {
      ...
      var spinner = inputQuantity.spinner({
        ...
        min: minNum,
        max: maxNum,
        step: 1, // Açık olarak 1'er artırmayı belirt
        ...
        spin: function(event, ui) {
          // Her spin olayında sadece 1'er artır/azalt
          var currentVal = parseInt($(this).val()) || minNum;

          if (ui.value > currentVal) {
            // Artırma - sadece 1 artır
            ui.value = Math.min(currentVal + 1, maxNum);
          } else {
            // Azaltma - sadece 1 azalt
            ui.value = Math.max(currentVal - 1, minNum);
          }
        }
      });
    });    
