-- ILISAN Commerce Final Seed Data Script (Fixed)
-- Tüm required field'lar dahil

-- Site ayarları tüm field'larla ekle
...
SELECT * FROM (
    ...
    ('CompanyFoundedYear', '2018', 5, 'Şirket Kuruluş Yılı', 1, '2024-01-01'),
    ('CompanyVision', 'Tam Bağımsız Türkiye için güvenlik çözümleri', 3, 'Şirket Vizyonu', 1, '2024-01-01'),
    ('CompanyMission', 'Güvenlik ve sağlığın sınırlarını zorlayan yenilikçi çözümler sunmak', 3, 'Şirket Misyonu', 1, '2024-01-01')
) AS S([Key], [Value], [Type], [Description], [IsActive], [CreatedDate])
WHERE NOT EXISTS (SELECT 1 FROM SiteSettings WHERE [Key] = S.[Key]);

-- Email şablonları ekle (sadece tablo varsa)
...
BEGIN
    ...
    PRINT 'Email templates eklendi.';
END

-- Ürün varyantları ekle (sadece mevcut ürünler için)
DECLARE @ProductCount INT;
SELECT @ProductCount = COUNT(*) FROM Products;

IF @ProductCount >= 1
BEGIN
    -- İlk ürün için varyantlar
    ...
    -- İkinci ürün için varyant
    IF EXISTS (SELECT 1 FROM Products WHERE Id = 2)
    BEGIN
        INSERT INTO [ProductVariants] ([ProductId], [VariantName], [Size], [Color], [Material], [PriceAdjustment], [StockQuantity], [MinStockLevel], [SKU], [IsActive], [IsDefault], [SortOrder], [CreatedDate])
        SELECT * FROM (
            VALUES 
...
SELECT * FROM (
    ...
    ('Askeri Malzemeler', 'TSK standartlarında askeri ürünler', 'askeri-malzemeler', 1, '2024-01-01')
) AS C([Name], [Description], [Slug], [IsActive], [CreatedDate])
WHERE NOT EXISTS (SELECT 1 FROM Categories WHERE [Slug] = C.[Slug]);

-- Rapor
PRINT '================================================';
PRINT '           ILISAN COMMERCE SEED DATA RAPORU';
PRINT '================================================';

SELECT 'Kullanıcı Sayısı' as [Tablo], COUNT(*) as [Toplam Kayıt] FROM Users;
SELECT 'Kategori Sayısı' as [Tablo], COUNT(*) as [Toplam Kayıt] FROM Categories;
SELECT 'Ürün Sayısı' as [Tablo], COUNT(*) as [Toplam Kayıt] FROM Products;
SELECT 'Ürün Varyant Sayısı' as [Tablo], COUNT(*) as [Toplam Kayıt] FROM ProductVariants;
SELECT 'Ürün Resim Sayısı' as [Tablo], COUNT(*) as [Toplam Kayıt] FROM ProductImages;
SELECT 'Site Ayar Sayısı' as [Tablo], COUNT(*) as [Toplam Kayıt] FROM SiteSettings;

IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'Sliders')
    SELECT 'Slider Sayısı' as [Tablo], COUNT(*) as [Toplam Kayıt] FROM Sliders;

IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'EmailTemplates')
    SELECT 'Email Template Sayısı' as [Tablo], COUNT(*) as [Toplam Kayıt] FROM EmailTemplates;
