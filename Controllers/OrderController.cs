...
namespace IlisanCommerce.Controllers
{
    public class OrderController : Controller
    {
        ...
        public async Task<IActionResult> Checkout(CheckoutViewModel model)
        {
            ...
            // Form nesnesini initialize et
            if (model.Form == null)
            {
                model.Form = new CheckoutFormModel();
            }
            if (model.Form.BillingAddress == null)
            {
                model.Form.BillingAddress = new AddressFormModel();
            }
            if (model.Form.ShippingAddress == null)
            {
                model.Form.ShippingAddress = new AddressFormModel();
            }

            // Guest kullanıcı için tüm alanlar zorunlu
            if (userId == null)
            {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    {
                        // Registered user
                        order.BillingAddressId = model.Form.BillingAddressId ?? 0;
                        order.ShippingAddressId = model.Form.ShippingAddressId ?? 0;
                        
                        // Kayıtlı kullanıcı için adres bilgilerini Order modelindeki alanlara da kaydet
                    ...
                    {
                        // Guest user - create addresses (UserId will be null)
                        var billingAddress = new Address
                        {
                            UserId = null, // Explicit null for guest orders
                        ...
                        // Misafir kullanıcı için adres bilgilerini Order modelindeki alanlara da kaydet
                        ...
                        if (!model.Form.SameAsShipping)
                        {
                            ...
                            // Misafir kullanıcı için teslimat adres bilgilerini Order modelindeki alanlara da kaydet
                        ...
                        else
                        {
                            ...
                            // Fatura adresi ile aynı ise teslimat adres bilgilerini de aynı yap
                    ...
                    foreach (var cartItem in cartItems)
                    {
                        var orderItem = new OrderItem
                        {
                            ...
                            // TotalPrice = cartItem.TotalPrice, // Remove this line as it's calculated
                    ...
                    // Status history simplified - order status is already set to Pending
            ...
            // ModelState.IsValid false olduğunda view'a geri dönerken model'i düzgün initialize et
            if (userId.HasValue)
            {
                model.UserAddresses = await _context.Addresses
                    .Where(a => a.UserId == userId.Value && a.IsActive)
            ...
            // Kargo hesaplamalarını yeniden yap
            var totalDesi = await _cartService.GetTotalDesiAsync(userId, sessionId);
            var shippingCost = await _cartService.CalculateShippingCostAsync(userId, sessionId);
            var cartTotal = cartItems.Sum(ci => ci.TotalPrice);
            
            model.TotalDesi = totalDesi;
            model.ShippingCost = shippingCost;
            model.CartTotal = cartTotal;
            model.GrandTotal = cartTotal + shippingCost;
        ...
        private async Task<IActionResult> InitializeIyzicoCheckoutFormAsync(Order order, CheckoutViewModel model, List<CartItem> cartItems)
        {
            try
            {
                ...
                var checkoutFormRequest = new CheckoutFormRequest
                {
                    ...
                    BuyerSurname = billingAddress.FullName.Split(' ').LastOrDefault() ?? "User",
                    BuyerEmail = model.Form.Email ?? "guest@ilisan.com.tr",
                    BuyerGsmNumber = billingAddress.Phone,
                    BuyerIdentityNumber = "11111111111", // Default for guest users
                    ...
                    // Basket items
                    BasketItems = cartItems.Select(item => new BasketItemModel
                    {
                        Id = item.ProductId.ToString(),
                        Name = item.Product.Name,
                        Category = item.Product.Category?.Name ?? "Ürün",
